...
**/
const messageEncoder = require('../../device/ev3/messageEncoder');
const CommandQueue   = require('../../device/ev3/CommandQueue').CommandQueue;
const constants      = require('../../device/ev3/constants');
...
exports.EV3Routes = class {
    ...
    connecting(req, res) {
        let ev3       = this._ev3;
        let connected = ev3.getConnected();
        let state     = {};
        if (connected) {
            state = this._ev3.getState();
        }
        res.send(JSON.stringify({
            connecting: ev3.getConnecting(),
            connected:  connected,
            state:      state
        }));
    }
