    ...
    run(commandId) {
        ...
        switch (commandId) {
            ...
            case mathModuleConstants.MATH_RANDOM:
                let rnd = vmData.getRecordFromAtOffset(['min', 'max']);
                rnd.max++;
            ...
            case mathModuleConstants.MATH_REMAINDER:
                let mod = vmData.getRecordFromAtOffset(['value', 'remainder']);
            ...
            case mathModuleConstants.MATH_POW:
                let pow = vmData.getRecordFromAtOffset(['base', 'exponent']);
                vmData.setNumberAtRet(Math.pow(pow.base, pow.exponent), 2);
                break;
            case mathModuleConstants.MATH_ODD:
                vmData.setNumberAtRet(((~~vmData.getRecordFromAtOffset(['value']).value & 1) === 1) ? 1 : 0);
                break;
            case mathModuleConstants.MATH_EVEN:
                vmData.setNumberAtRet(((~~vmData.getRecordFromAtOffset(['value']).value & 1) === 0) ? 1 : 0);
                break;
        }
    }
};
