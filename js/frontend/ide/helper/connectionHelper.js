...
exports.connectNXT = (settings, nxt) => {
    if (nxt.getConnectionCount() < settings.getNXTDeviceCount()) {
        if (platform.isWeb()) {
            nxtConnectionId++;
            dispatcher.dispatch('NXT.ConnectToDevice', 'NXT connection (' + nxtConnectionId + ')');
        } else {
            dispatcher.dispatch('Dialog.ConnectNXT.Show');
        }
        dispatcher.dispatch('Button.Device.NXT');
    } else {
        let lines = ['You\'ve reached the maximum number of connections.'];
        if (nxt.getConnectionCount() < nxtModuleConstants.LAYER_COUNT) {
            lines.push('You can change this setting in the <i>NXT</i> > <i>Device count</i> menu.');
        }
