...
exports.testComponentCall = function(it, opts) {
    it(
        opts.message,
        function(done) {
            ...
            let onFinished    = () => {
                    ...
                    vm.setCommands(program.getCommands()).run();
                    if (opts.type === 'const') {
                        assert.deepEqual(result[opts.property], opts.value);
                    } else if (opts.type === 'getNumber') {
                        ...
                        let recievedOpts = {};
                        recievedOpts[opts.property] = value;
                        assert.deepEqual(result, recievedOpts);
                    }
                    done();
                };
            let preProcessor = new PreProcessor({onGetFileData: onGetFileData, onFinished: onFinished});
